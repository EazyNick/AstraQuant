...
class StockTradingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.previous_portfolio_value > 0:
            short_term_reward = ((new_portfolio_value - self.previous_portfolio_value) / self.previous_portfolio_value) * 100
        else:
            short_term_reward = 0

        # 장기적 보상을 반영하도록 강화 (현재 가치 대비 초기 가치)
        long_term_reward = ((new_portfolio_value - self.initial_balance) / self.initial_balance) * 100 * 0.5
